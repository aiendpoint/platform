/**
 * OpenAPI 2.x/3.x -> /ai spec converter.
 * Rule-based, no LLM, runs entirely offline.
 *
 * Ported from registry/src/routes/convert.ts
 */
...
export function convertOpenApiToAi(raw: Record<string, unknown>): AiEndpointSpec {
  ...
  outer: for (const [path, pathItem] of Object.entries(paths)) {
    for (const method of HTTP_METHODS) {
      ...
      // Request body params (OpenAPI 3 only)
      if (isV3) {
        ...
        if (jsonSchema?.properties) {
          const reqFields = (jsonSchema.required as string[]) ?? []
          for (const [pName, pDef] of Object.entries(jsonSchema.properties as Record<string, { type?: string; description?: string }>)) {
            ...
            const req = reqFields.includes(pName) ? 'required' : 'optional'
